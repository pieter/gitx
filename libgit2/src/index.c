...
git_index_tree *git_index_tree__parse(const char **, const char *, git_index_tree *);
...
size_t git_index__parse_extension(git_index *index, const char *buffer, size_t buffer_size)
{
	...
	if (dest.signature[0] >= 'A' && dest.signature[0] <= 'Z') {
		...
		if (memcmp(dest.signature, INDEX_EXT_TREECACHE_SIG, 4) == 0) {
			...
			if (git_index__parse_tree(index, buffer + 8, dest.extension_size) < 0)
				return 0;
		}
	} else {
...
#define seek_forward(_increase) { \
	...
	/* Parse header */
	if (git_index__parse_header(&header, buffer) < 0)
		return GIT_EOBJCORRUPTED;
	...
	for (i = 0; i < index->entry_count && buffer_size > INDEX_FOOTER_SIZE; ++i) {
		size_t entry_size;
		entry_size = git_index__parse_entry(&index->entries[i], buffer, buffer_size);
	...
	while (buffer_size > INDEX_FOOTER_SIZE) {
		...
		extension_size = git_index__parse_extension(index, buffer, buffer_size);
